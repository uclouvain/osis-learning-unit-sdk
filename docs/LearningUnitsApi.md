...
[**learning_units_titles_read**](LearningUnitsApi.md#learning_units_titles_read) | **GET** /learning_units/{year}/titles/ | 
...
# **learning_units_titles_read**
> [LearningUnitAndClassTitle] learning_units_titles_read(year)



Return titles of learning units or classes if learning unit is a class
...
with osis_learning_unit_sdk.ApiClient(configuration) as api_client:
    ...
    code = "LDROI1001" # str, none_type |  (optional)
    accept_language = AcceptedLanguageEnum("en") # AcceptedLanguageEnum | The header advertises which languages the client is able to understand, and which locale variant is preferred. (By languages, we mean natural languages, such as English, and not programming languages.)  (optional)
    x_user_first_name = "X-User-FirstName_example" # str |  (optional)
    x_user_last_name = "X-User-LastName_example" # str |  (optional)
    x_user_email = "X-User-Email_example" # str |  (optional)
    ...
    # example passing only required values which don't have defaults set
    try:
        api_response = api_instance.learning_units_titles_read(year)
        pprint(api_response)
    except osis_learning_unit_sdk.ApiException as e:
        print("Exception when calling LearningUnitsApi->learning_units_titles_read: %s\n" % e)

    # example passing only required values which don't have defaults set
    # and optional values
    try:
        api_response = api_instance.learning_units_titles_read(year, lang=lang, codes=codes, code=code, accept_language=accept_language, x_user_first_name=x_user_first_name, x_user_last_name=x_user_last_name, x_user_email=x_user_email, x_user_global_id=x_user_global_id)
        pprint(api_response)
    except osis_learning_unit_sdk.ApiException as e:
        print("Exception when calling LearningUnitsApi->learning_units_titles_read: %s\n" % e)
```
...
[**[LearningUnitAndClassTitle]**](LearningUnitAndClassTitle.md)
