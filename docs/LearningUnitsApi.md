...
> [InlineResponse2001] learning_units_titles_read(year)
...
with osis_learning_unit_sdk.ApiClient(configuration) as api_client:
    ...
    # and optional values
    try:
        api_response = api_instance.learning_units_titles_read(year, lang=lang, codes=codes, accept_language=accept_language, x_user_first_name=x_user_first_name, x_user_last_name=x_user_last_name, x_user_email=x_user_email, x_user_global_id=x_user_global_id)
        pprint(api_response)
    except osis_learning_unit_sdk.ApiException as e:
        print("Exception when calling LearningUnitsApi->learning_units_titles_read: %s\n" % e)
```
